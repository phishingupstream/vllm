#!/usr/bin/env python3
"""Test harness for thinking token budget control.

Tests thinking token budgets against the vLLM endpoint.
Budget is passed via thinking_token_budget (native v0.19 parameter).

Usage:
    python3 tests/test_thinking_budget.py [--url URL] [--step N]
...
def send_anthropic_request(
    ...
    """Send a request via the Anthropic /v1/messages endpoint."""
    body: dict = {
        "model": "default",
        "messages": [{"role": "user", "content": prompt}],
        "max_tokens": max_tokens,
    ...
    if thinking_enabled:
        thinking_config: dict = {"type": "enabled"}
        if budget is not None:
            thinking_config["budget_tokens"] = budget
        else:
            thinking_config["budget_tokens"] = max_tokens - 1
    ...
    for block in data.get("content", []):
        if block.get("type") == "thinking":
            ...
            content_chars += len(block.get("text", ""))
...
def run_step6_anthropic(base_url: str) -> list[TestResult]:
    """Step 6: Anthropic endpoint thinking budget tests."""
    print("\n=== Step 6: Anthropic Endpoint (/v1/messages) ===\n")
    results = []

    test_cases = [
        ("anth_disabled", SIMPLE_PROMPT, None, False, "Thinking disabled"),
        ("anth_no_cap", MEDIUM_PROMPT, None, True, "Thinking enabled, no explicit budget"),
        ("anth_budget_2048", HARD_PROMPT, 2048, True, "Budget 2048"),
        ("anth_budget_512", HARD_PROMPT, 512, True, "Budget 512"),
    ]

    for name, prompt, budget, enabled, desc in test_cases:
        r = send_anthropic_request(base_url, prompt, budget=budget, thinking_enabled=enabled)
        r.name = name
        results.append(r)

        # Validation
        if not enabled and r.thinking_chars > 50:
            r.passed = False
            r.note = f"Thinking disabled but thinking_chars={r.thinking_chars}"
        if budget is not None and budget < 10000:
            max_chars = (budget + 5) * 6
            if r.thinking_chars > max_chars:
                r.passed = False
                r.note = f"OVER BUDGET: ~{r.thinking_tokens}t >> {budget}"

        print(
            f"  {name} (budget={budget}, enabled={enabled}): "
            f"thinking={r.thinking_chars}c/~{r.thinking_tokens}t, "
            f"content={r.content_chars}c, finish={r.finish_reason} "
            f"{'PASS' if r.passed else 'FAIL'} {r.note or desc}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Test thinking budget control")
    parser.add_argument(
        "--url", default=DEFAULT_URL, help="vLLM base URL"
    )
    parser.add_argument(
        "--step",
        type=int,
    ...
    all_results = []
    steps = range(7) if args.step is None else [args.step]
    ...
    for step in steps:
        if step == 0:
            ...
            all_results.extend(run_step6_anthropic(args.url))
